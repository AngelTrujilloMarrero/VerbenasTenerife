# VerbenasTenerife (25 ayuntamientos + 3 agregadores)
...
- Lógica: `src/lib/classifier.ts` (patrones + `partirPorDias` + `tipoDeEvento`),
  ...
  / `src/lib/icodvinos.ts` / `src/lib/lossilos.ts` / `src/lib/buenavista.ts` / `src/lib/laorotava.ts` / `src/lib/losrealejos.ts` / `src/lib/guimar.ts` / `src/lib/candelaria.ts` / `src/lib/elsauzal.ts` / `src/lib/santaursula.ts` / `src/lib/lamatanza.ts` / `src/lib/lavictoria.ts` / `src/lib/laguancha.ts` / `src/lib/sanmiguel.ts`, agregadores `src/lib/lagenda.ts` / `src/lib/tenerifesevive.ts` / `src/lib/canariasfiestas.ts`, agregador `src/lib/verbenas.ts`, PDFs en `src/lib/pdf.ts` (pdfjs-dist, gratis).
...
- El Sauzal: WP (REST 401), todo TEXTO web: `/actividad/slug` con ficha DÍA/HORA/LUGAR/ACTUACIONES + noticia del programa. PDFs fiestas escaneados -> OCR auto central. Solo año vigente.
...
- Tegueste DESCUBRE programas: cada ciclo lee /fiestas/ y procesa los PDF
  ...
  PDFs escaneados van al OCR auto central (`ocr-en-curso` en `/api/estado.json`).
...
Estado: 118 eventos en 24 municipios (verificado 13-sep-2026: La Victoria 8, San Miguel 2 futuras 27/29-sep, La Guancha 4, La Matanza 3, etc.). Santa Cruz: días flexibles, bailes sin hora con hora previa, penalty religioso solo sin mención explícita, dedup general con fusión. Chips y `pie-total` ya cuentan solo futuras.
...
Siguiente paso: 26º ayuntamiento o guardar en Firebase con el modelo `Event`.
...
Siguiente paso: 26º ayuntamiento (sin fuente oficial: Arafo, El Tanque, Garachico, Puerto de la Cruz, Santiago del Teide y Vilaflor).
