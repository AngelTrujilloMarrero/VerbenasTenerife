...
- Candelaria: WP REST posts cat 41 + mediateca PDF vigente (Agosto-2026). Noticias con programa inline (Nueva Línea). Fotos de programas por pueblo (año en nombre) -> OCR auto con validación día-semana.
...
Siguiente paso: Vilaflor (único sin fuente oficial) o guardar en Firebase con el modelo `Event`.
...
Siguiente paso: guardar en Firebase con el modelo `Event` (los 31 tienen fuente; solo Vilaflor va por agregador).
